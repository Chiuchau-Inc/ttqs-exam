...
個人名義提供的 **TTQS 匿名線上測驗**(給同學/一般考生)。考生免註冊、20 題 30 分鐘、交卷立即看成績與解析;
管理者在後台用 Dify 產題、看統計、匯出成績。

與同 repo `exam_app`(90 分鐘模擬考,含簡答/申論 AI 評分)是**兩個獨立服務**,分工:

| | quiz_app(本服務) | exam_app |
|--|--|--|
| 對象 | 同學/一般考生匿名快測 | 認真備考的學員 |
| 題型 | 選擇 + 是非 | 單選 + 簡答 + 申論 |
| Dify 角色 | **考前**出題(入題庫池) | **考後**評分 + 討論 |
| 評分位置 | 伺服器端 | 前端(單選)+ Dify(問答) |
...
- **70 分及格**(答對 14 題)
- 限時 **30 分鐘**,前端倒數,到時自動交卷;伺服器另有 40 分鐘 TTL 寬限
