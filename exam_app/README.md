...
考試網站：進網頁 →（60 分鐘計時）作答 → 交卷看結果 → 逐題檢討與討論。
全卷（單選+是非）前端本地即時計分；逐題檢討可經**後端代理**呼叫 Dify 討論「為何錯、正確觀念是什麼」。
...
| 區塊 | 每卷抽題 | 配分 | 計分方式 |
|------|----------|------|----------|
| 第一題 單選題 | 12 題 | ×5＝60 分 | 前端本地即時計分 |
| 第二題 是非題 | 8 題 | ×5＝40 分 | 前端本地即時計分 |

及格 60 分。題庫現有：單選 150、是非 40（另保留簡答 24、申論 12 於題庫檔，現行考卷不抽）。
...
- **全卷本地計分**（答案固定，公平一致、不耗 token）；Dify 僅用於逐題「討論」。
...
```
不設 `DIFY_API_KEY` 也能跑：計分不受影響，僅「討論這題」AI 功能停用。
